module ApplicationHelper
  def link_account
  	...
  	unless user_signed_in?
  	  html += link_to "My Account",new_user_registration_path,class: "phone-box  clear-box text-center"
    else
      ...
      # html += "<button type='button' class='btn btn-default dropdown-toggle' data-toggle='dropdown'> Action <span class='caret'></span> </button>"
  		html += "<ul class='dropdown-menu' role='menu'> <li>#{link_to 'Logout',destroy_user_session_path, method: :delete }</li></ul>"
  		html += "</div>"
     end
